"""
Converts all data directories in a folder called 'Data' into NWB files in a 
folder named 'Files'.
"""
import os
import pynwb
...
def create(identifier, subject_id, date, age = 'U', genotype = 'U', sex = 'U'):
    """
    :param identifier: ID
...
def lick_times(behavior_module):
    """
...
def wheel_moves(behavior_module):
    """
...
def make_file(curr_dir, identifier, age, genotype, sex, date):
    """
    ...
    :return:
    """
    nwb_file = create(identifier, age, genotype, sex, date)
    behavior_module = ProcessingModule('behavior', 'behavior module')
    nwb_file.add_processing_module(behavior_module)
    eye(behavior_module)
    face_nwb(behavior_module)
    lick_piezo(nwb_file)
    lick_times(behavior_module)
    spontaneous(nwb_file)
    wheel(nwb_file)
    wheel_moves(behavior_module)
    trial_table(nwb_file)
    sparse_noise(nwb_file)
    passive_stimulus(nwb_file)
    neural_data(nwb_file)
    filename = curr_dir + '/Files/Steinmetz2019_' + identifier + '.nwb'
...
def main():
    ...
    """
    curr_dir = os.getcwd()
    sess = pd.read_csv('sessInfo.tsv', sep='\t')
    subj = pd.read_csv('subjInfo.tsv', sep='\t')
    datasets = os.listdir(curr_dir + "/Data")
    datasets = [str(d) for d in datasets if len(str(d)) > 10]
    ...
    for d in datasets:
        ...
        make_file(curr_dir, d, age, genotype, sex, date)
        os.chdir(curr_dir)
