...
import numpy as np
from neuro.comp2.data import THData
...
class Neuron:
    ...
    session_stop_time  : float = None
    epoch_start_times  : list  = None
    epoch_stop_times   : list  = None
    ...
    def __post_init__(self):
        ...
        if self.data:     
            self.session_start_time = self.data.session.session_start_time if self.session_start_time is None else self.session_start_time
            self.session_stop_time  = self.data.session.session_start_time if self.session_start_time is None else self.session_stop_time
            self.epoch_start_times  = self.data.session.epoch_start_times  if self.epoch_start_times  is None else self.epoch_start_times
            self.epoch_stop_times   = self.data.session.epoch_stop_times   if self.epoch_stop_times   is None else self.epoch_stop_times
