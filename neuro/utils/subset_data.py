...
def subset_epoch_events(event_times, epoch_start_times, epoch_end_times):
    ...
    for ix in range(len(epoch_start_times)):
        EE = np.append(
            ...
            event_times[
                ...
                (event_times < epoch_end_times[ix])
                ],   
            axis = 0)
    return EE

def subset_epoch_data(data, event_times, epoch_start_times, epoch_end_times):
    """ 
    ...
    for ix in range(len(epoch_start_times)):
        epoch_ixs_ix = np.where((epoch_start_times[ix] < event_times) \
                                        & (event_times < epoch_end_times[ix]))
